...
<?php 
	...
	$query1 = mysql_query('SELECT `player_id` , `player_name` ,`country` ,`type` FROM '.$match.' WHERE `type`="BAT" ORDER BY `player_id`');
	if($query1){
		while($row1=mysql_fetch_assoc($query1)){
?>
			<li class='squad'><button class='batsman' name=<?php echo $row1['player_name'];?> title=<?php echo $row1['type'];?> id=<?php echo $row1['player_id'];?> > <?php echo $row1['player_name']; ?> </button></li></br>
<?php 
		}
	}else{
		echo "<strong style='color:red'>No Batsman Right Now</strong>";
	}
?>
